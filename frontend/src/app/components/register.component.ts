...
@Component({
  ...
  template: `
    <div class="auth-page">
      ...
      <div class="glass-card">
        ...
        <h2 class="title">REGISTRO <span class="highlight">TAKIS</span></h2>
        
        <form (submit)="onSubmit()">
          <div class="field">
            <label>NOMBRE COMPLETO</label>
            <input type="text" [(ngModel)]="form.name" name="name" required placeholder="Juan Pérez">
          </div>
          <div class="field">
          ...
          <div class="field">
            ...
            <input type="tel" [(ngModel)]="form.phone" name="phone" required placeholder="10 dígitos">
          </div>
          
          <div class="terms">
            <input type="checkbox" [(ngModel)]="form.terms" name="terms" required id="tc">
            <label for="tc">Acepto los términos y condiciones + aviso de privacidad</label>
          </div>
  ...
  styles: [`
    ...
    .glass-card { background: rgba(255,255,255,0.05); backdrop-filter: blur(10px); padding: 3rem; border-radius: 2rem; width: 100%; max-width: 450px; border: 1px solid rgba(255,255,255,0.1); margin-top: 2rem; }
...
export class RegisterComponent {
  ...
  onSubmit() {
    if (!this.form.terms) {
      this.toast.show('Debes aceptar los términos y condiciones.', 'info');
      return;
    }
    ...
    this.http.post('https://takis.qrewards.com.mx/api/index.php/auth/register', this.form).subscribe({
      next: (res: any) => {
